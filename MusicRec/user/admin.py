...
class adminUserMess(admin.ModelAdmin):
    # 将字段全部显示出来
    list_display = ("u_id", "u_name", "u_birthday", "u_gender",'u_province',"u_city", "u_type", "u_auth_status", "u_account_status",'u_dj_status','u_vip_type',)
    # 添加search bar，在指定的字段中searc
    search_fields = ("u_id", "u_name", "u_birthday", "u_gender",'u_province',"u_city", "u_type", "u_auth_status", "u_account_status",'u_dj_status','u_vip_type',)
    # 页面右边会出现相应的过滤器选项
    list_filter = ( "u_gender", "u_type", "u_auth_status", "u_account_status",'u_dj_status','u_vip_type',)
    # 排序
    ordering = ("-u_birthday",)
